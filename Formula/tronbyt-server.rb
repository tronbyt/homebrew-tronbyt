...
class TronbytServer < Formula
  ...
  head do
    url "https://github.com/tronbyt/server.git", branch: "main"
    depends_on "go" => :build
    depends_on "libwebm"
  end

  # To be removed once v2 is stable
  ...
  def install
    if build.head?
      ldflags = %W[
        ...
        -X tronbyt-server/internal/version.Commit=#{Utils.git_short_head}
        -X tronbyt-server/internal/version.BuildDate=#{Time.now.utc.iso8601}
      ]
  ...
  service do
    ...
    environment_variables(
      # These environment variables are specific to the Python (v1) application and are ignored by the
      # Go (v2) application.
  ...
  test do
    ...
    if build.head?
      ...
      File.open(log_file, "w") do |file|
        ...
        30.times do
          sleep 1
          break if log_file.read.include?("Application startup complete")
        end
        Process.kill("TERM", pid)
        Process.wait(pid)
      end
